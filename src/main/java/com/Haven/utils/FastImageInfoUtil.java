...
/**
 * 获取图片大小 FastImageInfoUtil
 *
 * @author HavenJust
 * @date 01:37 周三 11 五月 2022年
 */

public class FastImageInfoUtil {

    private int height;
    private int width;
    private String mimeType;

    public FastImageInfoUtil(File file) throws IOException {
        try (InputStream is = new FileInputStream(file)) {
            processStream(is);
        }
    }

    public FastImageInfoUtil(InputStream is) throws IOException {
        processStream(is);
    }

    public FastImageInfoUtil(byte[] bytes) throws IOException {
