...
public class CommonUtil {
    ...
    public static String getLocalPath() {
        ...
        if (System.getProperty("os.name").contains("Windows")) return (path.substring(1, path.lastIndexOf("target")) + "target/");
        return path.substring(0, path.lastIndexOf("target")) + "target/";
    }

    public static String getImagePath() {
        String path = Objects.requireNonNull(MineWebsiteApplication.class.getResource("")).getPath().replaceAll("file:", "");
        if (System.getProperty("os.name").contains("Windows")) return (path.substring(1, path.lastIndexOf("target")) + "target/image/").replaceAll("%20", " ");
        return (path.substring(0, path.lastIndexOf("target")) + "target/image/").replaceAll("%20", " ");
    }
